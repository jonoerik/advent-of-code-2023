...
class TileInterface(abc.ABC):
    ...
    def advance(self, all_tiles: dict[tuple[int, int], Self]) -> None:
        """Simulate a single step of the puzzle, with ON cells spreading outwards by 1. This state should have been
        pre-calculated by a call to calculate_next_state(). This is necessary, as all tiles, and all cells within
        them must advance simultaneously, rather than advancing one tile, then using that new state in the calculation
        of the adjacent tile's state."""
        pass
...
class Tile(TileInterface):
    ...
    def _on_in_neighbourhood(self, r: int, c: int, all_tiles: dict[tuple[int, int], TileInterface],
        ...
        for nr, nc in [(r-1, c), (r+1, c), (r, c-1), (r, c+1)]:
            ...
            if nr == -1:
                tr -= 1
                nr = len(self._cells) - 1
            elif nr == len(self._cells):
                tr += 1
                nr = 0
            elif nc == -1:
                tc -= 1
                nc = len(self._cells[0]) - 1
            elif nc == len(self._cells[0]):
                tc += 1
                nc = 0
    ...
    def calculate_next_state(self, all_tiles: dict[tuple[int, int], TileInterface], current_step: int) -> None:
        ...
        if all([self._cells[r][c] == CellState.BLOCKED or
                ...
                for r in range(len(self._cells)) for c in range(len(self._cells[0]))]):
...
class FilledTile(TileInterface):
    """A tile where all non-blocked cells are on or off in a strict checkerboard pattern. All cells alternate between
    on and off, and are the opposite state of their immediate neighbours."""
